...
class Settings {
    ...
    public function register_settings() {
        ...
        add_settings_field(
            'language',
            esc_html__( 'Dil / Language', 'el-doviz' ),
            [ $this, 'field_language' ],
            'ledoviz-turkish-exchange-rates-settings',
            'ledoviz_turkish_exchange_rates_general_section'
        );
    ...
    public function field_debug_mode() {
        ...
        echo '<label for="debug_mode">' . esc_html__( 'Hata ayıklama günlüğünü etkinleştir (uploads/ledoviz-turkish-exchange-rates/logs/debug.log dosyasına yazar).', 'el-doviz' ) . '</label>';
    }
    ...
    public function field_language() {
        ...
        echo '</select>';
        echo '<p class="description">' . esc_html__( 'Eklentinin önyüz ve panel dilini seçin. / Select the frontend and dashboard language of the plugin.', 'el-doviz' ) . '</p>';
    }
    ...
    public static function add_admin_menu() {
        ...
        add_submenu_page( 'el-doviz', esc_html__( 'Günlükler', 'el-doviz' ), esc_html__( 'Günlükler', 'el-doviz' ), 'manage_options', 'ledoviz-turkish-exchange-rates-logs', [ \ElDoviz\Admin\Page\LogsPage::class, 'render' ] );
    }
    ...
    public static function render_dashboard() {
        ...
        echo esc_html__( 'Bu eklenti, Türkiye Cumhuriyeti Merkez Bankası (TCMB) verilerini kullanarak döviz kurlarını gösterir. Eklenti aracılığıyla ziyaretçilerinize ait hiçbir kişisel veri toplanmaz, işlenmez veya üçüncü şahıslarla paylaşılmaz. KVKK uyumluluğu için sayfalarınıza "El Döviz Gizlilik ve KVKK" bileşenini veya [ledoviz_turkish_exchange_rates_privacy] kısa kodunu ekleyebilirsiniz.', 'el-doviz' );
        echo '</div>';
        echo '</div>';

        // Render Premium Navigation Tabs
        echo '<h2 class="nav-tab-wrapper">';
        echo '<a href="' . esc_url( admin_url( 'admin.php?page=ledoviz-turkish-exchange-rates&tab=usage' ) ) . '" class="nav-tab ' . ( 'usage' === $active_tab ? 'nav-tab-active' : '' ) . '">' . esc_html__( 'Kullanım Rehberi', 'el-doviz' ) . '</a>';
        echo '<a href="' . esc_url( admin_url( 'admin.php?page=ledoviz-turkish-exchange-rates&tab=settings' ) ) . '" class="nav-tab ' . ( 'settings' === $active_tab ? 'nav-tab-active' : '' ) . '">' . esc_html__( 'Genel Ayarlar', 'el-doviz' ) . '</a>';
        echo '<a href="' . esc_url( admin_url( 'admin.php?page=ledoviz-turkish-exchange-rates&tab=tools' ) ) . '" class="nav-tab ' . ( 'tools' === $active_tab ? 'nav-tab-active' : '' ) . '">' . esc_html__( 'Araçlar', 'el-doviz' ) . '</a>';
        echo '</h2>';
        ...
        if ( 'usage' === $active_tab ) {
            ...
            echo '<div class="ledoviz-turkish-exchange-rates-doc-card">';
            echo '<span class="ledoviz-turkish-exchange-rates-badge">Elementor Widget</span>';
            echo '<h3>' . esc_html__( 'El Döviz Kurları ve Trendler', 'el-doviz' ) . '</h3>';
            echo '<p>' . esc_html__( 'Para birimlerini dinamik yükseliş/düşüş okları (trend göstergeleri) ve yüzde değişimleriyle birlikte gösterir. Renkleri ve tasarımı tamamen özelleştirilebilir.', 'el-doviz' ) . '</p>';
            echo '</div>';
            ...
            echo '<li><code>ledoviz_turkish_exchange_rates_api_endpoints</code>: ' . esc_html__( 'TCMB ve BIST 100 API veri kaynağı adreslerini değiştirmek için kullanılır.', 'el-doviz' ) . '</li>';
            echo '</ul>';
            echo '</div>';
            ...
            submit_button( esc_html__( 'Önbelleği Temizle', 'el-doviz' ), 'primary', 'ledoviz_turkish_exchange_rates_clear_cache', false );
            echo '</form>';
            echo '</div>';
        }
